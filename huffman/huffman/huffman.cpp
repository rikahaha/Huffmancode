...
Node* BuildHuffmanTree(vector<Node*> vctNode)
{
    while (vctNode.size() > 1)                            //vctNode森林中树个数大于1时循环进行合并
    {
        ...
        Node* first = vctNode[1];    //取排完序后vctNode森林中频度最小的树根
        Node* second = vctNode[0];   //取排完序后vctNode森林中频度第二小的树根
